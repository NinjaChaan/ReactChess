...
void Asema::paivitaAsema(Siirto* siirto) {
	...
	if (nappula != NULL && (nappula == vs || nappula == ms) && (siirto->getLoppuruutu().getRivi() == 0 ||
		siirto->getLoppuruutu().getRivi() == 7)) {
...
std::vector<Siirto> Asema::annaRaakaSiirrotKillerTt(Siirto ttSiirto, Siirto killer1, Siirto killer2) {
	...
	std::stable_partition(lista.begin(), lista.end(), [ttSiirto, killer1, killer2]
	(const Siirto& s) -> bool {
		if (s == ttSiirto) return true;
		if (s == killer1) return true;
		if (s == killer2) return true;
		return false;
	});
	return lista;
}
...
bool Asema::mahdotonLoppupeli() {
	if (nappuloidenMaara > 4) {
		...
		if (mustia == 1) //vain kuningas jäljellä
		{
			if (nappulatJaljella[VL] > 0 && valkoisia == 2) return true; //vain kuningas ja lähetti
			if (nappulatJaljella[VR] > 0 && valkoisia == 2) return true; //vain kuningas ja ratsu
			if (nappulatJaljella[VR] == 2 && valkoisia == 3) return true; //vain kuningas ja 2 ratsua

		}
		else if (valkoisia == 1) //vain valkoinen kuningas jäljellä
		{
			if (nappulatJaljella[ML] > 0 && mustia == 2) return true; //vain kuningas ja lähetti
			if (nappulatJaljella[MR] > 0 && mustia == 2) return true; //vain kuningas ja ratsu
			if (nappulatJaljella[MR] == 2 && mustia == 3) return true; //vain kuningas ja 2 ratsua
		}
	}
	return false;
}
...
MinMaxPaluu Asema::iteratiivinenAlphaBeta2(const lautaDict& historia, int kello, int* maxSyvyys) {
	...
	for (auto s : viimePaluuLista) {
		...
		else if (s._parasSiirto.onkoPitkälinna()) {
			linnaVali = L" ";
		}
...
MinMaxPaluu Asema::iterAlphaBetaRoot2(int syvyys, std::vector<MinMaxPaluu>* paluuarvot, lautaDict historia, Siirto viimeParasSiirto) {
	...
	try {
		...
		//Siirrä viime paras siirto ensimmäiseksi
		for (auto i = siirrot.begin(); i != siirrot.end(); ++i) { //Etsi onko paras siirto listassa ja laita se listan eteen
...
double Asema::iterAlphaBeta2(int syvyys, int syvyysJuuresta, const lautaDict& historia, double alpha, double beta) {
	if (siirtokello.AikaLoppu()) throw std::exception();
...
double Asema::evaluoiNegamax() {
	if (siirtovuoro == 1) return -evaluoi();
	else return evaluoi();
}
...
double Asema::kuninkaanTurvallisuus() {
	...
	//lähetti kuninkaan lähellä +0.1
	if (onkoNappulaVieressa(valkeaKuningasRuutu, VL)) {
		arvo += 0.1;
	}
	//ratsu kuninkaan lähellä +0.1
	...
	//sotilaita kuninkaan edessä +0.05 per
	for (int i = -1; i <= 1; i++) {
...
double Asema::sotilaidenLiikeArvo() {
	//Sotilas lähempänä vastustajan laitaa +0.1 per askel 1
	//2 Sotilasta vierekkäin samalla rivillä -0.1 2
	//Sotilas blokattuna (ei pääse liikkumaan eteenpäin) -0.1 3
	//Sotilas yksin (ei ketään omaa nappulaa lähellä) -0.1 4
	//Jos toinen lähetti syöty: sotilas samalla värillä kun elossa oleva lähetti -0.1 
	//Lähetti tai hevonen suoraan sotilaan takana +0.1 6
	...
	for (int x = 0; x < 8; x++) {
		for (int y = 0; y < 8; ++y) {
			if (lauta[x][y] == vs) {
				arvo += (y - 1) * 0.1; // 1
				if (getNappulaRuudussa(x + 1, y) == vs) { // 2
					arvo -= 0.1;
				}
				if (getNappulaRuudussa(x, y + 1) != NULL) { // 3
					arvo -= 0.1;
				}
				//if (laskeNappulatVieressa(x, y, 0) == 0) { // 4 Hidas, eikä vaikuta pelin tulokseen
				//	arvo -= 0.1;
				//}
				if (getNappulaRuudussa(x, y - 1) == vl || getNappulaRuudussa(x, y - 1) == vr) { // 6
					arvo += 0.1;
				}
				if (onkoVapaaSotilas(x, y, 0)) { // 7
					arvo += 0.2;
				}
...
double Asema::linjat() {
	//tornilla, lähetillä, kuningattarella = +0.01 per mahdollinen liike
	return ((vDsiirrot + vTsiirrot + vLsiirrot) * 0.01) - ((mDsiirrot + mTsiirrot + mLsiirrot) * 0.01);

	//torni samalla rivillä kuin kuningatar = +0.05
}
